...
summary component appends SummaryLanguageInstruction to the summary block
prompt so the model writes the bullet items in that language; an empty value
appends nothing, keeping the prompt byte-identical and the LLM prefix cache
intact. The language changes only the item text — the bullet-list format and
the completion-signal role are unchanged.
`
...
- A summary block is required in EVERY response, even when no change blocks are emitted — including a response whose only blocks are ingest, go-src, shell, or go-test blocks. When no changes were made, use "No changes were needed." as the only bullet point.
...
// SummaryLanguageInstruction returns the instruction appended to
// SummaryBlockSystemPrompt when a summary language is configured
// (flags.SummaryLanguage, from the -summary-language flag or the
// "summary_language" config path). An empty language returns an empty
// string, so the unmodified prompt stays byte-identical and the LLM
// prefix cache is preserved. The instruction changes only the language
// of the item text; the bullet-list format and the block's
// completion-signal role are unchanged. See TheoryOfSummaryBlocks.
func SummaryLanguageInstruction(language string) string {
	if language == "" {
		return ""
	}
	return "\n**Summary Language:**\n\n- Write each summary bullet item in " + language + ". Keep the \"- \" bullet-list format and the single-phrase-per-item rule; only the language of the item text changes. Identifiers, file paths, and technical terms may remain in their original form when that is clearer.\n"
}
