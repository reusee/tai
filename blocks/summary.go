...
- A summary block is required in every response, even when no change blocks are emitted — including a response whose only blocks are ingest, go-src, shell, or go-test blocks. If no changes were made, generate a summary block with "No changes were needed." as the only bullet point.
