...
// parses its XML metadata, and returns the resulting Hunk.
...
- **Boundary matching**: The closing :::end marker MUST use the exact same boundary as the opening :::change marker. Closing :::change 徕珑 with :::end 栢彣 is a hard error that drops the entire block and loses your modifications. Always reuse the boundary you opened the block with; never copy a boundary from another block or from an example into the closing marker.
...
- **Boundary matching**: The closing :::end marker MUST use the exact same boundary as the opening :::change marker. A mismatched closing marker is a hard error and the entire block is dropped, losing your modifications. Always close a block with the same boundary you opened it with.
- The metadata is a self-closing XML tag: ` + "`<change op=\"...\" target=\"...\" file-path=\"...\" />`" + `
