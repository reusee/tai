...
3. 如果发现了新信息，就调用 update_user_profile 工具来更新用户画像。
   - 必须通过系统提供的函数调用机制进行调用，严禁在回复正文中直接写出类似于 "update_user_profile(...)" 的文本。
   - 更新时，提供你认为应该是当前最准确的用户画像项列表。
   - 系统会自动合并你的输入与现有记录，确保没有任何旧信息被意外删除。
   - 如果没有发现任何新信息，则不要调用工具。
   - 在提取和记录信息时，必须坚持高度确定性的事实原则：仅记录用户在对话中明确表达的事实，严禁记录任何缺乏根据的主观推测、直觉判断或过度推论。宁愿保持简洁的画像，也不要加入未经验证的猜测。
