...
const TheoryOfTitleButtons = `
Tab title button theory (cmd/tai):

- Every expanded tab's title row carries, right-aligned, one unicode
  button per operation that acts on that tab: the Output tab gets the
  section navigation and the sections collapse-all, the Tree tab gets
  the view cycling and the nodes collapse-all; the Logs tab carries no
  tab-specific operation and no buttons. The two rightmost cells stay
...
- Each button renders a single Han character as its label — two cells
  wide — so the visible label spans the press target; adjacent
  buttons carry no separator cells.
- The labels tile the toolbar span with no gaps, so the panel title
  ...
  cells stay outside the span, so the rule survives as the row's
  right margin.
- The renderer and the hit test share titleButtonLayout, so what is
  drawn is what is pressed. A press on a button runs its action
  through dispatchControlBar — the same action vocabulary the menu
  bar and the key dispatch share — and preempts the ordinary press
  handling, so a press on a button never toggles or focuses the tab.
  A press on the title row outside the buttons keeps the ordinary
  strip semantics. Hover renders the button reversed, affordance
  only, like the menu bar.
- A tab too narrow for the buttons drops the buttons that do not
  fit, rightmost first; the reserved cells stay.
`

// titleButton is one button of a tab title row: its unicode glyph and
// the control bar action the press runs.
type titleButton struct {
	Glyph  string
	Action controlBarAction
}

// The title buttons join the shared action vocabulary. See
...
// The button glyphs: plain, colorable characters.
const (
	titleButtonPrev     = "上"
	titleButtonNext     = "下"
...
// operation that acts on the tab. See TheoryOfTitleButtons.
...
// caller holds t.mu. See TheoryOfTitleButtons.
