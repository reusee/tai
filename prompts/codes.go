...
- **Temporal Awareness**: Strictly observe the sequence and chronology of the input (e.g., dates, version markers, log order). Explicitly differentiate between historical artifacts and the current operative state. When designs evolve, identify and anchor your analysis on the "latest design," treating previous iterations as context for rationale rather than active requirements.
