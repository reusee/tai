...
source. GoSrcBlockSystemPrompt teaches this snapshot behavior: a fetch
of a just-modified symbol returns the old source by construction, and
the model must neither read that as a failed change — an applied change
block that reported no error took effect — nor use a fetch to verify an
applied change or to re-apply one. Any divergence between the cached
snapshot and the filesystem is repaired only by the next loop, which
re-reads the current filesystem state. The prompts recommend the full
import path as the qualifier form because it is unique over loaded
packages, unlike a declared name or path suffix that may match several
same-named packages.
`
