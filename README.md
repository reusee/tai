...
Every command runs in a terminal UI by default when stdout is a terminal: an Output tab for model output, a Tree tab for the run's session tree (the pipeline yields the full tree after every recorded occurrence, so the tab renders — and projects — the same tree the pipeline writes), and a Logs tab for log records. When stdout is redirected to another program or a file (for example `tai next | tee .AI`), the default is plain command-line output: the TUI discards stdout, so a piped consumer would receive nothing. The `-tui` flag forces the TUI explicitly; the `-cli` flag switches to plain command-line output.
## Configuration
