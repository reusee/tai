...
is not repeated here. New test functions follow the same preference at
file level: they join an existing test file of their package whenever one
is suitable, because a package's tests belong together; ChangeBlockPrompt's
section carries this guidance.
...
- **Applied Means Effective**: A change block that reported no apply error took effect; apply errors are reported back as explicit feedback, so the absence of an error is the confirmation. Do not re-read a file you just modified (an ingest block) to verify an applied change — the re-read burns a round and cannot confirm anything the apply result did not already confirm.

**Prefer Precise Modifications:**
Prefer precise modifications (MODIFY, ADD_BEFORE, ADD_AFTER, DELETE, REPLACE, INSERT_BEFORE, INSERT_AFTER) over WRITE whenever the change is small or localized. WRITE replaces the entire file, which is token-expensive, requires re-reviewing every line, and risks altering unrelated code. Reserve WRITE for creating new files or when the majority of the file content is changing. When the change adds new test functions, prefer writing them into an existing test file of the same package (via ADD_BEFORE or ADD_AFTER targeting a declaration of that file); create a new test file only when no existing file in the package is suitable.
