...
ConfigGlobals is the taiconfigs-side injection point for config globals; the
mechanism lives in configs.TheoryOfConfigGlobals. The default provider
returns an empty map; modules that want to inject values override this
provider on their own Module struct with a method that builds the globals
map from Go constants, variables, or computed values. In Go, when an outer
struct and an embedded struct both define a method with the same name, the
outer struct's method shadows the embedded one, so the override takes
