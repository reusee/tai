...
func TestApplyHunksSkipDependency(t *testing.T) {
	tmpDir := t.TempDir()
	oldCwd, _ := os.Getwd()
	os.Chdir(tmpDir)
	defer os.Chdir(oldCwd)
	...
	// Create a dummy go.mod to define module root
	if err := os.WriteFile("go.mod", []byte("module test"), 0644); err != nil {
		t.Fatal(err)
	}

	// Absolute path outside of tmpDir
	outsideFile := "/tmp/some_dependency.go"
	if outsideFile == filepath.Join(tmpDir, "some_dependency.go") {
		// Just in case tmpDir is /tmp
		outsideFile = "/etc/hosts"
	}

	aiFile := filepath.Join(tmpDir, "test.AI")
	aiContent := []byte(`[[[ MODIFY Foo IN ` + outsideFile + `
func Foo() {}
]]]`)
	if err := os.WriteFile(aiFile, aiContent, 0644); err != nil {
		t.Fatal(err)
	...
	// AI file should be empty (hunk removed because it was skipped)
	aiNewContent, _ := os.ReadFile(aiFile)
	if len(bytes.TrimSpace(aiNewContent)) > 0 {
		t.Errorf("hunk not removed from AI file: %s", string(aiNewContent))
	}
}