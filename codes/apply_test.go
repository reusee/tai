...
func TestApplyHunksXmlDiffWithCDATAClosingInContent(t *testing.T) {
	tmpDir := t.TempDir()
	oldCwd, _ := os.Getwd()
	os.Chdir(tmpDir)
	defer os.Chdir(oldCwd)
...
`)
	...
	aiFile := filepath.Join(tmpDir, "test.go.AI")
	// The body contains ]]> inside a string, which would break standard XML parsers.
	aiContent := []byte(`<xml>
<change op="MODIFY" target="Foo" file-path="` + targetFile + `">
<![CDATA[
func Foo() {
	...
	s := string(newContent)
	if !strings.Contains(s, `new ]]> here`) {
	...
	if strings.Contains(string(aiRemaining), "<change") {
		t.Errorf("change element not removed from AI file")
	}
}
