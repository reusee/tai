...
下面是一次完整的交互记录，其形式为按应用顺序排列的事件流：每个事件一行「名称 kind=操作 type=类型 author=作者 parent=父节点 time=时间」（空字段省略），其下缩进的行是该操作写入节点时的完整内容；kind=delete 的事件表示该节点及其子树被删除。归属关系由 parent 字段表达：attempt节点下属它消耗的输入、它产生的输出、它生成的块及其处理结果、以及过程事件节点；loop节点下属该轮的会话节点。
