...
- Guide task partitioning: if the previous round was truncated due to output volume or attempting too many changes, explicitly identify what changes were attempted, evaluate whether they are sound, and advise the next round to complete an initial manageable subset of those changes first and use a continue block to partition the remaining work across subsequent rounds, rather than repeatedly emitting an oversized response that triggers truncation again. Also advise the next round to batch independent work into the same round — parallel context fetches, unrelated commands, unrelated change blocks — so the partitioned work needs fewer rounds.
